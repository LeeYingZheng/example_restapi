...
def login_username(creds: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
...
def login_email(creds: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
